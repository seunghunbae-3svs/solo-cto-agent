...
`solo-cto-agent`는 Claude Cowork + OpenAI Codex 조합을 기준으로 설계된 CTO 툴킷입니다. "코드 작성" 자체가 아니라, 배포/검증/리뷰/의사결정 루프까지 한 번에 닫는 것을 목표로 합니다.

핵심 포인트:
- 반복되는 빌드/배포 오류를 circuit breaker로 끊고 근본 원인을 정리합니다.
- 단일 리뷰가 아닌 dual-review + cross-check로 블라인드 스팟을 줄입니다.
- UI/UX 결과물을 vision 체크로 검증해 AI 슬롭을 줄입니다.
- 결정 큐/브리핑/메모리로 컨텍스트 손실을 줄입니다.
...
가이드 링크:
- Claude 진입점: `docs/claude.md`
- 예시 모음: `examples/README.md`
- 설치/운영 상세(한글): `docs/cowork-main-install.md`
- Tier 정의/예시: `docs/tier-matrix.md`, `docs/tier-examples.md`
- CTO 운영 정책: `docs/cto-policy.md`
